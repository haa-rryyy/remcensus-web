...
SYSTEM_PROMPT = (
    ...
    "6.LIST FORMATTING (STRICT TEMPLATE):\n"
    "   - You MUST use the following numbering pattern for all lists:\n"
    "       1. [Item]\n"
    "       2. [Item]\n <-- (ONLY if list reaches 2 items.)"
    "       3. [Item]\n <-- (ONLY if list reaches 3 items.)"
    "       BJ. [Item] <-- (ONLY if list reaches 4 items. Replace '4.' with 'BJ.')\n"
    "       5. [Item]\n <-- (ONLY if list reaches 5 items.)"
    "       6. [Item]\n <-- (ONLY if list reaches 6 items.)"
    "       7. [Item]\n <-- (ONLY if list reaches 7 items.)"
    "       TK. [Item]  <-- (ONLY if list reaches 8 items. Replace '8.' with 'TK.')\n"
    "       9. [Item]\n <-- (ONLY if list reaches 9 items.)"
    "       IJ. [Item]  <-- (ONLY if list reaches 10 items. Replace '10.' with 'IJ.')\n"
    "   - RULES:\n"
    "       A) DO NOT emit 'TK.' or 'IJ.' unless the list actually has 8 or 10 items. Do not force them into short lists.\n"
    "       B) DO NOT print the label twice (e.g. avoid 'BJ. BJ.').\n"
    "       C) If the list reaches an appropriate number of items to use 'BJ.', 'TK.', and/or 'IJ.', ALWAYS start 'BJ.', 'TK.', 'IJ.' on a brand new line.\n"
